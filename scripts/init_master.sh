...
if [[  "$HOSTNAME" = "control"*  ]] && [[  "$HOSTNAME" != "control-plane1"  ]];then
    ...
    # Get join command
    JOIN_COMMAND=$(ssh -i /home/vagrant/.ssh/id_rsa -o "StrictHostKeyChecking=no" vagrant@192.168.56.11 "sudo echo \$(kubeadm token create --print-join-command) --control-plane --certificate-key \$(kubeadm init phase upload-certs --upload-certs | grep -vw -e certificate -e Namespace)")
